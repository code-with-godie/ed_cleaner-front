...
`
const AddService= ({setOpen,setProducts}) => {
  ...
  const [product,setProduct] = useState({img:'',title:"",price:'',priceNumber:'',description:''});
  ...
  const onSubmit = async e =>{
    ...
    try {
      setLoading(true);
      const res = await postData('/services',product,token);
      if(res){
...
theme: "dark",
    ...
    setTimeout(()=>{
        setProducts(prev => [res?.service,...prev]);
      setOpen(false);  
...
// transition: Bounce,
  ...
  return (
      ...
      <Form onSubmit ={onSubmit} >
        ...
        <ControlWrapper>
            <Control className='active' disabled = {disabled} > {loading ? <LoadingAnimation/>:'ADD SERVICE'} </Control>
        </ControlWrapper>
      </Form>
    </WrapperTwo>
    <ToastContainer/>
