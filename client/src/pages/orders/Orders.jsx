  ...
  useEffect(()=>{
    if(data){
      const newData = data?.orders.map(item => {
        const {user:{firstName,lastName,profilePic},amount,address,paymentType,_id,createdAt,receipt_url} = item;
        return ({firstName,lastName,profilePic,amount,address,paymentType,_id,date:createdAt,receipt_url})
      })
      setRows(newData)
    }
  },[data])
