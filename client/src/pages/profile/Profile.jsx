...
`
const Profile = () => {
    const {currentUser:loggedInUser,token}= useSelector(state => state.user)
  ...
  const handleChange = e =>{
   ...
   const value = e.target.value;
    ...
    try {
      setLoading(true);
      const res  = await updateData(`/users/profile/${loggedInUser?._id}`,user,token);
      if(res){
        dispatch(login({user:res?.user,token}))
